...
def create_in_mongo(request):
    some_random_text = "".join(
        [random.choice("fdksgvjhkfdbvjhsfdbvhjsd") for _ in range(3)])

    saved = Entry(
        blog=[
            Blog(
                name=some_random_text,
                text=some_random_text
            )
        ],
        headline="some random text"
    ).save()
...
def all_entries(request):
    ...
    return HttpResponse(
        "| ".join([timestamp.strftime("%m-%d-%Y %H:%M:%S") for timestamp in
                   blogs_timestamps])
    )
